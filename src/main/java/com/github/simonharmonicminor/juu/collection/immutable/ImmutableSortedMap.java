...
public interface ImmutableSortedMap<K, V> extends ImmutableMap<K, V> {
     ...
     * empty.) The returned map is backed by this map, so changes in the returned map are reflected in
     * this map, and vice-versa. The returned map supports all optional map operations that this map
     * supports.
     *
     * <p>The returned map will throw an {@code IllegalArgumentException} on an attempt to insert a
     ...
     *                                  fromKey} or {@code toKey} cannot be compared to keys currently in the map.
     ...
     *                                  map itself has a restricted range, and {@code fromKey} or {@code toKey} lies outside the
     *                                  bounds of the range
     */
    ImmutableSortedMap<K, V> subMap(K fromKey, K toKey);

    /**
     * Returns a view of the portion of this map whose keys are strictly less than {@code toKey}. The
     ...
     * fromKey}. The returned map is backed by this map, so changes in the returned map are reflected
    ...
    /**
     * Converts this immutable sorted map to mutable one. Creates new object, so its mutations does
